    ...
    return (
        <Link href={href}>
            <div className={` cursor-pointer flex items-center ${
                isCollapsed ? "justify-center py-4" : 'justify-start px-8 py-4'} hover:text-blue500 hover:bg-blue-100 gap-3 transition-colors ${
  ...
  return (
    <div className={sideBarClassNames}>
        ...
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            <div>logo</div>
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>LECAJSTOCK</h1>
        ...
        <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10"`}>   
            <p className="text-center text-xs text-gray-500">&copy; 2026 LECAJSTOCK.</p>
        </div>
    </div>
  )
}
